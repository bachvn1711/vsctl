...
document.getElementById("add-form").addEventListener("submit", async event => { if (event.submitter?.value === "cancel") { event.preventDefault(); dialog.close(); return; } event.preventDefault(); const form = new FormData(event.currentTarget); const payload = Object.fromEntries(form.entries()); payload.writable = form.get("writable") === "on"; const response = await fetch("/api/v1/signals", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); const result = await response.json(); if (!response.ok) { log(result.error || "Signal could not be added.", true); return; } dialog.close(); log(`Added ${result.path}.`); await load(); });
